...
export const createDiagnostic = async (req: Request, res: Response) => {
  try {
    ...
    if (!vehicleId || !description) {
      return res.status(400).json({ error: 'Faltan campos obligatorios: vehicleId y description.' });
    }
    ...
    console.error(error);
    res.status(500).json({ error: 'Error al registrar el diagnóstico técnico.' });
  }
};
...
export const getPriorityDiagnostics = async (_req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Error al obtener diagnósticos prioritarios' });
  }
};
...
export const confirmDiagnosticDelivery = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { reportImageUrl } = req.body;
    
    if (!id || typeof id !== 'string') {
      return res.status(400).json({ error: 'ID de diagnóstico faltante o inválido.' });
    }

    const updatedDiagnostic = await DiagnosticService.confirmDelivery(id, reportImageUrl);
    res.json(updatedDiagnostic);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Error al confirmar la entrega del diagnóstico.' });
  }
};
...
export const acceptDiagnosticReport = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: 'Error al confirmar la recepción del reporte.' });
  }
};
