...
export const ToolTracker = () => {
  ...
  const handleAddLoan = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
  ...
  return (
    <div className="glass-card p-8 rounded-3xl border border-slate-800 mt-8 shadow-2xl bg-[#0E131F]/40 backdrop-blur-md">
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          ...
          className={`p-3 rounded-2xl transition-all ${
            showForm ? "bg-red-500/20 text-red-500" : "bg-blue-600 text-white shadow-lg shadow-blue-500/20"
          }`}
        >
      ...
      {showForm && (
        <form onSubmit={handleAddLoan} className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-3 animate-in fade-in zoom-in">
          <input
          ...
          />
          <button type="submit" className="bg-orange-500 text-[#070B14] font-black uppercase text-[10px] rounded-xl hover:bg-orange-400 transition-colors">
            REGISTRAR
          </button>
        </form>
      )}
      ...
      <div className="space-y-3">
        {loans.map((loan) => (
          <div key={loan.id} className="flex justify-between items-center bg-slate-950/40 p-5 rounded-3xl border border-slate-800/50 hover:border-orange-500/30 transition-all">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-2xl bg-slate-800 flex items-center justify-center text-orange-500">
                <Wrench size={20} />
              </div>
              <div>
                <p className="text-sm font-black text-white uppercase tracking-tight">{loan.toolName}</p>
                <p className="text-[10px] font-bold text-slate-500 uppercase italic">{loan.borrowerName}</p>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <button
                onClick={() => window.open(`https://wa.me/${loan.borrowerPhone}?text=Hola ${loan.borrowerName}, recordatorio de devolución: ${loan.toolName}`)}
