...
import { createWhatsAppLink, formatCriticalAlertMessage } from "../lib/messages";
...
export const CriticalAlerts = () => {
  ...
  useEffect(() => {
    const fetchCriticalAlerts = async () => {
      try {
        const { data } = await api.get<CriticalAlert[]>("/diagnostics/critical");
        setAlerts(data);
      } catch (error) {
        console.error(error);
  ...
  const handleNotifyOwner = (phone: string | undefined, ownerName: string, plate: string) => {
    if (!phone) return;
  ...
  const handleMarkDelivered = async (id: string) => {
    ...
    try {
      const { data: updatedDiagnostic } = await api.patch<CriticalAlert>(`/diagnostics/${id}/deliver`);
      setAlerts((current) =>
        current.map((alert) =>
          alert.id === id
            ? {
      ...
      if (hideManaged) {
        ...
        window.setTimeout(() => {
          ...
          setRemovingAlertIds((current) => current.filter((remId) => remId !== id));
        }, 300);
      }
    } catch (error) {
      console.error(error);
    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-900/50 shadow-black/20 shadow-xl overflow-hidden">
      <div className="flex flex-col gap-4 border-l-4 border-red-600 bg-slate-950/40 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-lg font-black text-white">Vehículos en ROJO</h3>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:items-end sm:flex-row sm:gap-6">
          <label className="flex items-center gap-3 rounded-full border border-slate-700 bg-slate-900/90 px-4 py-2 text-xs uppercase tracking-[0.25em] text-slate-300 transition">
            ...
            >
              <span className={`inline-block h-5 w-5 rounded-full bg-white shadow transition-transform duration-300 ${hideManaged ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </label>
      ...
      <div className="divide-y divide-slate-800">
        ...
        {!loading && !error && visibleAlerts.length === 0 && (
          <div className="p-6 text-slate-400">No hay alertas críticas pendientes.</div>
        )}

        {!loading && !error && visibleAlerts.map((alert) => {
          ...
          const statusLabel = isAccepted ? 'CONFIRMADO' : isSent ? 'ENVIADA' : 'CRÍTICO';
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  <p className={`font-mono text-sm ${isAccepted ? 'text-sky-200 opacity-70' : isSent ? 'text-slate-400 opacity-70' : 'text-slate-200'}`}>{alert.vehicle.plate}</p>
                  {isAccepted && <span className="text-sky-400">✅✅</span>}
                  {!isAccepted && isSent && <span className="text-slate-400">✅</span>}
                </div>
                <span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.35em] ${statusClasses}`}>
                  {statusLabel}
                </span>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="space-y-1 text-slate-400 text-xs">
                  <p className="text-[11px] uppercase tracking-[0.25em] text-slate-500">Placa</p>
                  <p className={`font-mono text-sm ${isSent ? 'text-slate-400 opacity-70' : 'text-slate-100'}`}>{alert.vehicle.plate}</p>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  {!isSent && (
                    ...
                    >
                      {pendingDeliveryId === alert.id ? 'Procesando...' : 'Confirmar Entrega'}
                    </button>
                  )}
                </div>
              </div>
